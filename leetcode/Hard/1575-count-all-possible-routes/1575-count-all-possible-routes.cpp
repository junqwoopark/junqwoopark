...
public:
    ...
    int dfsWithMemo(const vector<int>& locations, int city, int finish, int remainFuel, vector<vector<int>>& memo) {
        if (remainFuel < 0) {
            return 0;
        }
        ...
        for (int nextCity = 0; nextCity < locations.size(); nextCity++) {
            if (nextCity != city && remainFuel >= abs(locations[nextCity] - locations[city])) {
                total = (total + dfsWithMemo(locations, nextCity, finish, remainFuel - abs(locations[nextCity] - locations[city]), memo)) % MOD;
            }
        }
    ...
    int countRoutes(vector<int>& locations, int start, int finish, int fuel) {
        ...
        return dfsWithMemo(locations, start, finish, fuel, memo);
    }
};